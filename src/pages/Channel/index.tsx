...
import { Button, message, QRCode, Switch } from 'antd';
...
const handleUpdate = async (fields: FormValueType) => {
  ...
  try {
    await updateItem(`/channels/${fields._id}`, fields);
    hide();
  ...
  return (
    <PageContainer>
      <ProTable<API.ItemData, API.PageParams>
        ...
        toolBarRender={() => [
          (access.canSuperAdmin || access.canCreateChannel) && (
            <Button
              type="primary"
              key="primary"
              onClick={() => {
        ...
        request={async (params, sort, filter) =>
          queryList('/channels', { ...params, isOnline: activeKey }, sort, filter)
        }
        columns={columns}
        rowSelection={
          access.canSuperAdmin && {
      ...
      {selectedRowsState?.length > 0 && (
        ...
        >
          {(access.canSuperAdmin || access.canDeleteChannel) && (
            <DeleteButton
              onOk={async () => {
      ...
      {(access.canSuperAdmin || access.canUpdateChannel) && (
        <Update
