...
const handleUpdate = async (fields: FormValueType) => {
  ...
  try {
    await updateItem(`/regulation-agencies/${fields._id}`, fields);
    hide();
    ...
    <PageContainer>
      <ProTable<API.ItemData, API.PageParams>
        ...
        request={async (params, sort, filter) =>
          queryList('/regulation-agencies', params, sort, filter)
        }
        columns={columns}
        rowSelection={
          access.canSuperAdmin && {
      ...
      {selectedRowsState?.length > 0 && (
        ...
        >
          {(access.canSuperAdmin || access.canDeleteRegulationAgency) && (
            <DeleteButton
              onOk={async () => {
      ...
      {(access.canSuperAdmin || access.canCreateRegulationAgency) && (
        <Create
      ...
      {(access.canSuperAdmin || access.canUpdateRegulationAgency) && (
        <Update
