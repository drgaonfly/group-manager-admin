...
const handleUpdate = async (fields: FormValueType) => {
  ...
  try {
    await updateItem(`/comments/${fields._id}`, fields);
    hide();
...
const handleRemove = async (ids: string[]) => {
  ...
  try {
    ...
    const response = await removeItem('/comments', {
      ids,
    });
...
const TableList: React.FC = () => {
  ...
  const columns: ProColumns<any>[] = [
    {
      title: intl.formatMessage({ id: 'pages.comment.customer' }),
      dataIndex: ['customer', 'username'], // 修改为 username
      render: (dom, entity) => {
        return (
          <a
            onClick={() => {
  ...
  return (
    <PageContainer>
      <ProTable<API.ItemData, API.PageParams>
        ...
        request={async (params, sort, filter) => queryList('/comments', params, sort, filter)}
        pagination={{
          defaultPageSize: 10,
