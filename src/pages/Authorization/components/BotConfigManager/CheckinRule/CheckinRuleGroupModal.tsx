...
import { Modal, Button, Space, message, Tag, Card, Row, Col, Spin, Empty, Popconfirm } from 'antd';
...
const CheckinRuleGroupModal: React.FC<Props> = ({ open, onClose, bot, group }) => {
  // 两种类型各维护一个 rule（null = 未配置）
  ...
  const handleEdit = (type: RuleType) => {
    setEditingType(type);
    setEditingRecord(rules[type]); // null = 新建，有值 = 编辑
    setFormOpen(true);
  };
...
export default CheckinRuleGroupModal;
