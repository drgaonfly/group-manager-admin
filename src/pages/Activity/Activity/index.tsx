...
const handleAdd = async (fields: API.ItemData) => {
  ...
  try {
    await addItem('/activities', { ...fields });
    hide();
...
const handleUpdate = async (fields: FormValueType) => {
  ...
  try {
    await updateItem(`/activities/${fields._id}`, fields);
    hide();
  ...
  return (
    <PageContainer>
      <ProTable<API.ItemData, API.PageParams>
        ...
        toolBarRender={() => [
          (access.canSuperAdmin || access.canCreateActivity) && (
            <Button
              type="primary"
              key="primary"
              onClick={() => {
        ...
        request={async (params, sort, filter) =>
          queryList('/activities', { ...params, isOnline: activeKey }, sort, filter)
        }
        columns={columns}
        rowSelection={
          access.canSuperAdmin && {
      ...
      {selectedRowsState?.length > 0 && (
        ...
        >
          {(access.canSuperAdmin || access.canDeleteActivity) && (
            <DeleteButton
              onOk={async () => {
      ...
      {(access.canSuperAdmin || access.canUpdateActivity) && (
        <Update
