...
const handleAdd = async (fields: API.ItemData) => {
  ...
  try {
    await addItem('/transfers', { ...fields });
    hide();
...
const handleUpdate = async (fields: FormValueType) => {
  ...
  try {
    await updateItem(`/transfers/${fields._id}`, fields);
    hide();
  ...
  return (
    <PageContainer>
      ...
      {selectedRowsState?.length > 0 && (
        ...
        >
          {(access.canSuperAdmin || access.canDeleteTransfer) && (
            <DeleteButton
              onOk={async () => {
      ...
      {(access.canSuperAdmin || access.canUpdateTransfer) && (
        <Update
