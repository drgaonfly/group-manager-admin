...
const handleAdd = async (fields: API.ItemData) => {
  ...
  try {
    await addItem('/wallet-deal-records', { ...fields });
    hide();
...
const handleUpdate = async (fields: FormValueType) => {
  ...
  try {
    await updateItem(`/wallet-deal-records/${fields._id}`, fields);
    hide();
...
const handleRemove = async (ids: string[]) => {
  ...
  if (!ids) return true;
  try {
    await removeItem('/wallet-deal-records', {
      ids,
    });
    hide();
    message.success(
...
const TableList: React.FC = () => {
  ...
  const columns: ProColumns<API.ItemData>[] = [
    ...
    {
      title: intl.formatMessage({ id: 'isOperativeOnAdmin' }),
      dataIndex: 'isOperativeOnAdmin',
      render: (text, record) => {
        return record.isOperativeOnAdmin
          ? intl.formatMessage({ id: 'yes' })
          : intl.formatMessage({ id: 'no' });
      },
    },
    {
      title: intl.formatMessage({ id: 'walletDealType' }),
      dataIndex: 'type',
      valueType: 'select',
      valueEnum: {
        collection: { text: '归集' },
  ...
  return (
    <PageContainer>
      <ProTable<API.ItemData, API.PageParams>
        ...
        toolBarRender={() => [
          (access.canSuperAdmin || access.canCreateWalletDealRecord) && (
            <Button
              type="primary"
              key="primary"
              onClick={() => {
        ...
        request={async (params, sort, filter) =>
          queryList('/wallet-deal-records', { ...params, isOnline: activeKey }, sort, filter)
        }
        columns={columns}
        rowSelection={
          access.canSuperAdmin && {
      ...
      {selectedRowsState?.length > 0 && (
        ...
        >
          {(access.canSuperAdmin || access.canDeleteWalletDealRecord) && (
            <DeleteButton
              onOk={async () => {
      ...
      {(access.canSuperAdmin || access.canUpdateWalletDealRecord) && (
        <Update
