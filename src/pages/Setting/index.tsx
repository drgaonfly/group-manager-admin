...
const handleUpdate = async (fields: FormValueType) => {
  ...
  try {
    await updateItem(`/settings/${fields._id}`, fields);
    hide();
...
const handleRemove = async (ids: string[]) => {
  ...
  try {
    ...
    const response = await removeItem('/settings', {
      ids,
    });
  ...
  return (
    <PageContainer>
      <ProTable<API.ItemData, API.PageParams>
        ...
        request={async (params, sort, filter) => queryList('/settings', params, sort, filter)}
        columns={columns}
        rowSelection={
          access.canSuperAdmin && {
